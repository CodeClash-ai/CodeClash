"""
Unit tests for BattleCode23Arena.
...
from codeclash.arenas.battlecode23.battlecode23 import BattleCode23Arena, RoundResult, SimulationMeta
...
class TestBattleCode23Validation:
    """Tests for BattleCode23Arena.validate_code()"""

    @pytest.fixture
    def arena(self, tmp_log_dir, minimal_config):
        """Create BattleCode23Arena instance with mocked environment."""
        config = minimal_config.copy()
        config["game"]["name"] = "BattleCode23"
        config["game"]["sims_per_round"] = 10
        config["players"] = [
            {"name": "p1", "agent": "dummy"},
            {"name": "p2", "agent": "dummy"},
        ]
        arena = BattleCode23Arena.__new__(BattleCode23Arena)
    ...
    def test_valid_submission(self, arena, mock_player_factory):
        """Test that a valid BattleCode23 submission passes validation."""
...
class TestBattleCode23SimulationParsing:
    """Tests for BattleCode23Arena._parse_simulation_log()"""
    ...
    def test_parse_team_a_wins(self, arena, tmp_log_dir):
        ...
        log_file.write_text(
            "[server] Game starting\n[server] mysubmission (A) wins\nReason: Team A conquered 75% of islands.\n"
        )
    ...
    def test_parse_team_b_wins(self, arena, tmp_log_dir):
        ...
        log_file = tmp_log_dir / "sim_0.log"
        log_file.write_text(
            "[server] Game starting\n[server] mysubmission (B) wins\nReason: Team B conquered 75% of islands.\n"
        )
    ...
    def test_parse_alternating_team_positions(self, arena, tmp_log_dir):
        ...
        log_file_0.write_text("[server] mysubmission (A) wins\nReason: Team A won.\n")
        ...
        log_file_1.write_text("[server] mysubmission (A) wins\nReason: Team A won.\n")
...
class TestBattleCode23Results:
    """Tests for BattleCode23Arena.get_results()"""
    ...
    def _create_sim_log(self, round_dir, idx: int, winner_team: str):
        """Helper to create a simulation log file."""
        log_file = round_dir / f"sim_{idx}.log"
        content = f"[server] mysubmission ({winner_team}) wins\nReason: Team won by conquering islands.\n"
    ...
    def test_get_results_with_ties(self, arena, tmp_log_dir):
        ...
        simulations = []
        # Alice wins 4, Bob wins 3, 3 ties (no winner line)
        for idx in range(10):
            if idx < 4:
                self._create_sim_log(round_dir, idx, "A")
                simulations.append(SimulationMeta(idx=idx, team_a="Alice", team_b="Bob", log_file=f"sim_{idx}.log"))
...
class TestBattleCode23Config:
    """Tests for BattleCode23Arena configuration and properties."""

    def test_arena_name(self):
        """Test that arena has correct name."""
        assert BattleCode23Arena.name == "BattleCode23"

    def test_submission_path(self):
        """Test that submission path is correct."""
        assert BattleCode23Arena.submission == "src/mysubmission"

    def test_default_args(self):
        """Test default arena arguments."""
        assert BattleCode23Arena.default_args["maps"] == "maptestsmall"
    ...
    def test_initialization_with_two_players(self, tmp_log_dir, minimal_config):
        ...
        with patch.object(BattleCode23Arena.__bases__[0], "__init__", return_value=None):
            arena = BattleCode23Arena.__new__(BattleCode23Arena)
            arena.config = config
            arena._round_result = None
    ...
    def test_initialization_fails_with_wrong_player_count(self, tmp_log_dir, minimal_config):
        ...
        with pytest.raises(AssertionError, match="two-player game"):
            with patch.object(BattleCode23Arena.__bases__[0], "__init__", return_value=None):
                BattleCode23Arena(config, tournament_id="test", local_output_dir=tmp_log_dir)


class TestBattleCode23SimulationMetadata:
...
class TestBattleCode23RoundResult:
